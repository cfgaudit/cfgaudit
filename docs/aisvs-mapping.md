...
| **C9.6.4** Secrets not exposed in the model-observable context | **CFG007** (`env` secret), **CFG016** (credential helper in project settings), **CFG050/CFG054** (MCP `env`/`headers` secret), **CFG031** (instruction names a secret file), **CFG037/CFG038** (commands read / exfiltrate secrets). |
