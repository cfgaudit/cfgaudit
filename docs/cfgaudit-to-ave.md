...
## Coverage (59 `static_detection` records)
...
Rechecked 2026-09-25 against **AVE-2026-00081 through AVE-2026-00083**, the three records added since. 00081 (a task-scoped capability outliving its episode) and 00083 (a guardrail that executes but never evaluates its verdict) are both `runtime_observed`. **00082 is static and lands on a shape this crosswalk reports as a gap**: a local skill name collision resolved by load order, which is CFG102's subject. It is still not taken, for a narrower reason than before — see Direction 3. The rule added since v1.14.0, CFG109, is reported as a gap: no record covers a declared configuration that does not load.
...
| CFG094 `.cursor/permissions.json` `autoRun.allow_instructions` · CFG103 Codex `features.guardianv2.classifier_instructions` (that finding only) | AVE-2026-00076 natural-language steering of an approval classifier subagent |
...
| CFG102 two committed skills claiming one name | Name shadowing. AVE-2026-00017 is explicitly MCP *server* identity and AVE-2026-00066 is registry squatting on names a model hallucinates; neither fits. AVE-2026-00082 is much closer but asserts silent shadowing across roots of differing trust, which this rule does not establish — see Direction 3 |
...
| **Automated security reviewer switched off or blunted by config** | CFG103 (Codex `features.guardianv2` `enabled = false`, raised `review_threshold`) | open — AVE-2026-00063 is explicitly a bypassed **human**-approval step, and Guardian v2 is an automated reviewer whose escalation feeds a blocking review. The `classifier_instructions` finding of the same rule does have a class (AVE-2026-00076); switching the reviewer off entirely does not |
| **Repository grants the agent browser or desktop-application access** | CFG106 (Codex `[browser_use]` per-origin `full_cdp_access`, `[computer_use]` app access) | open — a committed file decides which origins the agent may drive with full DevTools access and which desktop applications it may control. Not filed as AVE-2026-00063: whether a prompt is skipped is unverified (the value is observable in the effective config, but the tools live in the client app), so the class would assert more than the detection does |
...
| A **local name collision** inside one repository, where two committed skills claim one name | CFG102 | Narrowed on 2026-09-25 rather than closed. AVE-2026-00082 now covers a local skill name collision resolved by load order, which is the same shape, but it asserts two things CFG102 does not establish: that the shadowing is **silent**, and that the collision spans **roots of differing trust**. Measured for Claude Code, a duplicate inside one repository is resolved by qualification, not silently shadowed, and the cross-root case needs a machine-wide view a repository-scoped scanner does not have. What remains uncovered is the same-root collision, per agent, where the loser is never reported |

| A **declared configuration that never loads**, so the guardrails a repository states are absent while its files read as configured | CFG109, and the discarded-file case of CFG012 | No record covers it, and the whole set was read to be sure. AVE-2026-00083 is the nearest in effect — a protection that executes but never evaluates — but it is `runtime_observed` and scoped to a mechanism's own comparison logic, while this is a file the agent never loads. AVE-2026-00063 is a gate a configuration *removes*; here the configuration declares nothing that takes effect. Measured both directions: Codex logs "Invalid configuration; using defaults" and runs, Claude Code discards a settings file over one top-level type mismatch and starts the session |
...
*Mappings are class-level behavioral equivalence, not asserted identity. Re-checked 2026-09-25 against the current AVE record set, which now runs to **AVE-2026-00083** (00081 and 00083 are runtime behaviours; 00082 is static and narrows, without closing, the CFG102 gap), and before that on 2026-08-23 against **AVE-2026-00080** (00078–00080 are all multi-agent runtime behaviours, none of them a config surface). The 2026-08-14 pass, described below, checked through **AVE-2026-00077**; the git tag still reads v1.1.0 and the CHANGELOG's released sections stop at v1.3.0, so the records past 00070 are only visible in `records/`, not in a release. This pass mapped one of cfgaudit's five new rules (CFG098 → AVE-2026-00062) and, because seven new records landed, five rules that had no class before: CFG005, CFG046, CFG071 and CFG099 → AVE-2026-00073; CFG082 → AVE-2026-00071; CFG018 → AVE-2026-00072; CFG094 → AVE-2026-00076. CFG071 was missed on the first pass, which read AVE-2026-00073's title rather than its description; the description names a cleartext model or provider base URL as one of the three manifestations it covers. CFG100, CFG101 and CFG102 are deliberately unmapped, with reasons in Direction 1, and the last two are reported back as gaps. The 2026-08-23 pass added CFG104 and CFG105 → AVE-2026-00063 and CFG103 → AVE-2026-00076 (one finding of three), and left CFG106 unmapped with its reason in Direction 1 and Direction 3. cfgaudit CFG001–CFG109; CFG107, CFG108 and CFG109 are all deliberately unmapped, with reasons in Direction 1 and Direction 3. Re-check on the next AVE release. A machine-readable form in AVE's own crosswalk schema (`cfgaudit-to-ave.json`) exists for upstream contribution to their `crosswalks/` directory.*
