...
| [CFG089](docs/rules/CFG089.md) | warn | `.github/copilot/settings.json` (and its gitignored twin `settings.local.json`) auto-installs third-party plugins — `enabledPlugins` loads a plugin's hooks/commands/MCP on session start, an `extraKnownMarketplaces` source without a full-SHA `sha`/`ref` is unpinned, and `autoUpdate` on an entry refreshes its plugins every session (CFG055's threat model in Copilot's file; `warn` because a third of real files register the marketplace they publish, not because committability is in doubt) | LLM03 |
