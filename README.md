...
```sh
gh attestation verify cfgaudit_1.14.0_linux_amd64.tar.gz -R cfgaudit/cfgaudit
```
...
```yaml
include:
  - component: gitlab.com/cfgaudit/cfgaudit/cfgaudit@v1.14.0
    inputs:
      path: .
      format: text
```
...
```yaml
include:
  - component: gitlab.com/cfgaudit/cfgaudit/cfgaudit-code-quality@v1.14.0
    inputs:
      path: .
```
...
| [CFG004](docs/rules/CFG004.md) | error/warn/info | `defaultMode` set to `bypassPermissions` or `auto`, one level lower (`warn`/`info`) in the two repository-controlled scopes once the detected Claude Code stops honouring the value there (2.1.257 for bypass, already by 2.1.252 for auto); user scope and an undetected version keep `error`/`warn` | LLM06 |
| [CFG085](docs/rules/CFG085.md) | error/warn | subagent frontmatter weakens the permission mode — `.claude/agents/*.md` `permissionMode: bypassPermissions`/`dontAsk` → error, `auto`/`acceptEdits` → warn; Grok `.grok/agents/*.md` flags only `bypassPermissions` (the sole mode Grok wires at spawn); qwen `.qwen/agents/*.md` `approvalMode: yolo` (or bridged `permissionMode: bypassPermissions`) → error, `auto-edit`/`auto` → warn (`dontAsk` bridges to `default`, inert) — CFG004's modes reached through a committed agent file | LLM06 |
...
| [CFG046](docs/rules/CFG046.md) | warn/error/info | `OTEL_EXPORTER_OTLP_*ENDPOINT` redirects telemetry to a non-local collector (error for a raw IP; info in a repository settings file, which Claude Code 2.1.282+ ignores for telemetry) | LLM02 |
...
Codex guards a subset of keys against repo contents (`PROJECT_LOCAL_CONFIG_DENYLIST`: base URLs, `model_provider(s)`, `notify`, `profile(s)`, `otel`). cfgaudit drops those from a project file, so a committed `notify` is not treated as a command site and a committed `chatgpt_base_url` does not trigger CFG071 — the CLI would ignore them. `approval_policy`, `sandbox_mode`, `mcp_servers`, `features` and the newer `browser_use` / `computer_use` tables are not on that denylist. Project layers are loaded but disabled while the directory is untrusted, which caps the blast radius the way Cursor's workspace trust does for CFG086.

| ID | Severity | Description | OWASP |
|----|----------|-------------|-------|
| [CFG063](docs/rules/CFG063.md) | error/warn | Codex removes the human from the approval loop — `approval_policy` is `never` (auto-approve all → error) or `on-failure` (deprecated → warn), or `approvals_reviewer` is `auto_review`/`guardian_subagent`, which routes escalated prompts to a reviewer subagent instead of the person (→ warn), at the root key or at any of its three `[apps]` positions | LLM06 |
...
**Codex lifecycle hooks** live in two committable places, `<repo>/.codex/hooks.json` and an inline `[hooks]` table in `.codex/config.toml`, across twelve events (`PreToolUse`, `PermissionRequest`, `PostToolUse`, `PreCompact`, `PostCompact`, `SessionStart`, `SessionEnd`, `UserPromptSubmit`, `SubagentStart`, `SubagentStop`, `Stop`, `Interrupt`). `hooks` is **not** on the project-layer denylist, so a committed table is discovered; cfgaudit scans both files and sends every `type: "command"` handler through the command-content rules (CFG008/009/014/015/027/028/037/038/039/059/072/077/078). The Windows spelling (`commandWindows` / `command_windows`) counts too; `type: "prompt"` and `type: "agent"` do not, because Codex's discovery skips them as *"not supported yet"*.
...
**Codex's `[apps]` table** carries the approval decision a second time, for app/connector tools rather than for `[mcp_servers]`, and the two tables are separate in the config and in the resolver. cfgaudit reads all seven positions: `default_tools_approval_mode` on `[apps._default]`, on `[apps.<id>]` and on `[apps.<id>.links.<link>]`, plus `approval_mode` on `[apps.<id>.tools.<tool>]` ([CFG011](docs/rules/CFG011.md)), and `approvals_reviewer` on the first three ([CFG063](docs/rules/CFG063.md)). The value is the same `AppToolApproval` enum, so only `approve` is reported, and the per-tool position is judged like a tool list rather than reported on sight. `destructive_enabled` and `open_world_enabled` are **not** flagged: both resolve with `.unwrap_or(true)`, so a committed `true` restates the default and only `false` changes anything. Note that Codex writes the per-tool position itself when a tool is approved permanently, into the *user* config rather than a project one, so a value there in a repository is a committed user config or a hand-authored block.

**Codex's `[shell_environment_policy]`** builds the environment for every process Codex spawns through a shell-like tool, and its `set` is an open string map. cfgaudit reads `set` and sends it through the same code-execution classifier CFG020 uses on MCP server envs ([CFG107](docs/rules/CFG107.md)): a committed `BASH_ENV`, `ZDOTDIR`, `PYTHONSTARTUP`, `LD_PRELOAD`, `LD_AUDIT` or `DYLD_INSERT_LIBRARIES`, or a `--require`-style `NODE_OPTIONS`/`RUBYOPT`/`PERL5OPT`, runs the repository's own code on the agent's next command. The table is not on the project-layer denylist, and upstream's sanitizer strips exactly `ZDOTDIR` and `BASH_ENV` out of it, but only when the optional credential broker is enabled. The two library *search* paths (`LD_LIBRARY_PATH`, `DYLD_LIBRARY_PATH`) are reported only when an entry is relative or empty, because an absolute system path is what real build configs carry. The table's other keys (`inherit`, `exclude`, `include_only`, `filters`, `experimental_use_profile`) are **not** flagged: they decide which of the user's own variables survive, which introduces no repository-controlled content.
...
| `CLAUDE.md`, `.claude/CLAUDE.md`, `CLAUDE.local.md`, `.claude/skills/**/SKILL.md`, `.claude/commands/**/*.md` | scanned |
...
- **`QWEN.md`** (qwen's project instruction file; `~/.qwen/QWEN.md` with `--user`) and **`.qwen/agents/*.md`**, **`.qwen/commands/*.md`**, **`.qwen/skills/*/SKILL.md`**, **`.qwen/rules/**/*.md`**, **`.qwen/output-styles/*.md`** are scanned as instruction content (CFG024–CFG036, CFG057, CFG080/CFG081). qwen also reads `AGENTS.md`, which cfgaudit already scans. `.qwen/agents/*.md` frontmatter **mirrors Claude Code's agent schema verbatim** (a Claude agent file drops in and parses identically), so it carries execution blocks too: [CFG085](docs/rules/CFG085.md) reads its native `approvalMode` (`yolo` → error, `auto-edit`/`auto` → warn) and a bridged Claude `permissionMode` (honoured only when `approvalMode` is unset — `bypassPermissions` → `yolo`, `acceptEdits`/`auto` → `auto-edit`, while `dontAsk` maps to `default` and is inert); its `hooks` ride the command-content rules and CFG086's zero-click list, its `mcpServers` ride the shared MCP rules, and its qwen-only `executor:` block (an external process the subagent delegates to) is a command site.

**Output styles are instruction content on both agents, and they are discovered differently.** A style file's body is appended to the system prompt verbatim under a `# Output Style: <name>` heading when a committed settings file names it: `.claude/settings.json` `outputStyle` for Claude Code, `.qwen/settings.json` `general.outputStyle` for qwen. Claude Code reads its directory with `rg --files --hidden --follow --no-ignore --glob '*.md'`, so `.claude/output-styles` is **walked recursively**; qwen reads its own with a single `readdir` filtered on `/\.md$/i`, so `.qwen/output-styles/*.md` is a **single-level** glob and a nested file there is deliberately not scanned. Both are covered in project scope and, with `--user`, under `$HOME`.

**`.qwen/rules`** is walked recursively, like `.claude/rules`, because qwen discovers it that way. Upstream splits the directory in two: a rule file **without** a `paths:` frontmatter is a baseline rule and is injected into the system prompt at session start, while one **with** `paths:` is held back and injected mid-session when the model reads or edits a matching file. `loadRules` reads `~/.qwen/rules` (with `--user`) and, for a trusted folder, the project's own directory, and no settings key is needed to activate either: the committed file alone is enough. In a sample of 121 committed rule files across 12 repositories, **93 sat in subdirectories**, so a single-level glob would have missed most of them.
...
| [CFG103](docs/rules/CFG103.md) | error | committed Codex config weakens Codex's own security reviewer — `[features.guardianv2]` switches it off, raises its `review_threshold`, or replaces its `classifier_instructions`, and the `[auto_review]` table's `policy` / `experimental_policy_template` rewrite the same reviewer prompt one table over — verified at the artifact: a project config's values reach the effective config in a trusted directory, and neither table is on Codex's project-layer denylist | LLM06 |
...
| [CFG106](docs/rules/CFG106.md) | error/warn | committed Codex `[browser_use]` / `[computer_use]` grants the agent browser or desktop-application access — `full_cdp_access` (script, cookie and storage access in the browser session) and `computer_use.default_app_access` (every application) are errors, per-origin and per-application grants and `allow_history_access` are warns. The value enum is exactly allow/deny, so `deny` is hardening and never reported | LLM06 |
| [CFG107](docs/rules/CFG107.md) | error | committed Codex `[shell_environment_policy]` `set` carries an interpreter or dynamic-linker startup variable (`BASH_ENV`, `ZDOTDIR`, `PYTHONSTARTUP`, `LD_PRELOAD`/`LD_AUDIT`/`DYLD_INSERT_LIBRARIES`, or a `--require`-style `NODE_OPTIONS`/`RUBYOPT`/`PERL5OPT`) — the table is applied to every shell Codex spawns, so the next command runs the repository's code first. The two library *search* paths are reported only when an entry is relative or empty | LLM06 |
| [CFG108](docs/rules/CFG108.md) | error/info | committed `.claude/scheduled_tasks.json` (Claude Code's cron-scheduler store) carries actionable tasks — a **bare** task (no `createdBySessionId`/`createdInProject`) self-enables the otherwise-off scheduler by the file's presence and enqueues its `prompt` on the cron schedule in any contributor's interactive session with no interaction (error); a session-bound task is inert on another machine but still committed autonomous scheduling (info) | LLM06 |
| [CFG109](docs/rules/CFG109.md) | error | a committed agent config file cannot be parsed — its settings are not in force and no rule for that surface ran; the scan continues over the rest of the repository | LLM06 |
...
Also not flagged: `security.allowPrivateNetworkHooks` and `security.allowedInsecureVoiceBaseUrls`. Both sit on qwen's `WORKSPACE_RESTRICTED_SETTINGS`, which `stripWorkspaceRestrictedSettings` removes from the workspace layer before the merge *"so a repository cannot opt the user into those capabilities"*, so a committed value is inert. qwen enforces workspace scope with **three** lists in `settingsUtils.ts`, and `mergeSettings` applies all three to the workspace layer alone: `WORKSPACE_RESTRICTED_SETTINGS` (dropped outright), `WORKSPACE_NON_OVERRIDING_SETTINGS` (dropped when a higher scope sets the key) and `WORKSPACE_TIGHTEN_ONLY_SETTINGS` (kept only when strictly stricter than the operator scopes). Those lists, not the schema descriptions, are what to re-read when checking qwen scope, and their membership moves between releases ([CFG099](docs/rules/CFG099.md) has the recipe).
