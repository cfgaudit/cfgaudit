...
```sh
gh attestation verify cfgaudit_1.11.0_linux_amd64.tar.gz -R cfgaudit/cfgaudit
```
...
```yaml
include:
  - component: gitlab.com/cfgaudit/cfgaudit/cfgaudit@v1.11.0
    inputs:
      path: .
      format: text
```
...
```yaml
include:
  - component: gitlab.com/cfgaudit/cfgaudit/cfgaudit-code-quality@v1.11.0
    inputs:
      path: .
```
...
- **`.grok/config.toml`** `[mcp_servers]` (TOML, snake_case; the transport is inferred from `command` vs `url`, there is no `type` field) rides the shared MCP rules (CFG010–CFG021, CFG049–CFG059). A project config contributes only `[mcp_servers]`/`[plugins]`/`[permission]`; `[ui]`/`[sandbox]`/`[telemetry]`/`[model.*]` load only from `~/.grok/config.toml`, so cfgaudit does not read them from a scanned repo (they would be false positives). and `[plugins]` ([CFG100](docs/rules/CFG100.md)) of the three tables a project config may contribute
...
| LLM03 | [Supply Chain Vulnerabilities](https://owasp.org/www-project-top-10-for-large-language-model-applications/2025/LLM03_2025-Supply_Chain.html) | CFG010, CFG014, CFG052, CFG055, CFG074, CFG086, CFG089, CFG098, CFG100 |
