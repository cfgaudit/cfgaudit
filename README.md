...
| [CFG050](docs/rules/CFG050.md) | error | a committed `env` or `headers` block contains a hardcoded secret (vendor key pattern, secret-like name, or auth header with a literal credential) — MCP servers, a Copilot `type: "http"` hook, and an `extraKnownMarketplaces` entry, whose headers Claude Code sends with same-origin archive downloads | LLM02 |
