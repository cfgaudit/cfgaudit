...
```sh
gh attestation verify cfgaudit_1.10.0_linux_amd64.tar.gz -R cfgaudit/cfgaudit
```
...
```yaml
include:
  - component: gitlab.com/cfgaudit/cfgaudit/cfgaudit@v1.10.0
    inputs:
      path: .
      format: text
```
...
```yaml
include:
  - component: gitlab.com/cfgaudit/cfgaudit/cfgaudit-code-quality@v1.10.0
    inputs:
      path: .
```
...
| ID | Severity | Description | OWASP |
|----|----------|-------------|-------|
| [CFG086](docs/rules/CFG086.md) | error | committed agent hook runs on a zero-click event — Cursor `.cursor/hooks.json` `workspaceOpen`, Copilot `.github/hooks/*.json` `sessionStart`, or Grok `.grok/hooks/*.json` `SessionStart` — executes on every teammate who opens the repo, before they ask the agent for anything (cross-agent analogue of CFG047/CFG067) | LLM03 |
...
**Gemini's project agents live in `.gemini/agents/*.md`**, which its docs list as *"Project-level … Shared with your team"*. cfgaudit scans them as instruction content and decodes the frontmatter's `mcpServers` mapping onto the shared MCP rules, folding Gemini's `http_url` spelling into the URL the rules read, so a cleartext endpoint (CFG049) or a secret-bearing `headers` value (CFG050) declared in a committed agent file is caught. Discovery mirrors Gemini's own loader rather than a plain glob: a single directory read with **no recursion**, `.md` only, and names starting with `_` skipped, because reporting an `_draft.md` would claim a finding in a file Gemini never loads.

Three fields in that frontmatter are **not** modelled yet and are tracked separately: the per-server `trust` flag (verified to skip the confirmation prompt in a trusted folder), inline literals in the `auth` block, and a cleartext `agent_card_url`.
